...
class VoiceControMovelNode(Node):
    ...
    def __init__(self, name):
        ...
        if self.machine_type == 'JetRover_Acker':
            self.get_logger().info(
                '控制指令: 左转 右转 前进 后退(Voice command: turn left/turn right/go forward/go backward)'
            )
        else:
            self.get_logger().info(
            ...
            if self.words is not None:
                ...
                if self.words == '前进' or self.words == 'goforward':
                    ...
                    if self.machine_type == 'JetRover_Acker':
                        ...
                        twist.angular.z = -0.8
                elif self.words == '过来' or self.words == 'comehere' and self.machine_type != 'JetRover_Acker':
                    #                    self.play('come')
                    if 270 > self.angle > 90:
                        twist.angular.z = -1.0
                        self.time_stamp = time.time() + abs(
