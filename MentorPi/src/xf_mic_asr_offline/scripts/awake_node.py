...
class CircleMic:
    ...
    def __init__(self, port, flag_pub, angle_pub):
        self.serialHandle = serial.Serial(None,
                                          115200,
                                          serial.EIGHTBITS,
                                          serial.PARITY_NONE,
                                          serial.STOPBITS_ONE,
    ...
    def switch_mic(self, mic="mic6_circle"):
        ...
        param = {"type": "switch_mic", "content": {"mic": "mic6_circle"}}
    ...
    def set_wakeup_word(self, str_pinyin=AWAKE_WORD):
    ...
    def send_data(self, header, args):
        ...
        self.serialHandle.write(
            packet)  # 发送主控消息(Send messages to the main controller)

    # 发送数据(send data)
    def send(self, header, args):
        self.serialHandle.write([
            0xa5, 0x01, 0x01, 0x04, 0x00, 0x00, 0x00, 0xa5, 0x00, 0x00, 0x00,
            0xb0
        ])  # 发送握手请求(send handshake request)
        while True:
            result = None
            ...
            if recv_data == header_[0]:
                ...
                if recv_data == header_[1]:
                    ...
                    if recv_data == header_[2]:
                        ...
                        self.serialHandle.read((recv_data[1] << 8
                                                | recv_data[0]) + 1)
                        ...
                        time.sleep(0.1)
                        self.serialHandle.write([
                            0xa5, 0x01, 0x01, 0x04, 0x00, 0x00, 0x00, 0xa5,
                            0x00, 0x00, 0x00, 0xb0
                        ])  # 继续发送发送握手请求(send the handshake request again)
                            ...
                            if m is not None:
                                ...
                                if m is not None:
                                    ...
                                    if self.flag_pub is not None and self.angle_pub is not None:
                                        ...
                                        angle = self.val_map(
                                            angle, 0, 360, 360, 0
                                        ) + 240  # 和圆形兼容(compatible with the circle)
                                        if angle >= 360:
                                            angle -= 360
                                        msg = Int32()
                                        msg.data = int(angle)
...
class AwakeNode(Node):
    ...
    def __init__(self, name):
        ...
        self.declare_parameter('awake_word', AWAKE_WORD)
        ...
        self.create_service(SetString, '~/set_wakeup_word',
                            self.set_wakeup_word_srv)
