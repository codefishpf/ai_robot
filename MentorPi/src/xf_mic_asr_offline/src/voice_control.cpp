...
std::wstring utf8_to_wstring(const std::string& utf8) {
    ...
    // 保存当前locale设置
    std::string prev_locale = std::setlocale(LC_CTYPE, nullptr);
    ...
    if (len == static_cast<size_t>(-1)) {
        std::setlocale(LC_CTYPE, prev_locale.c_str());  // 恢复locale
        return L"";
    }
    
    std::wstring wstr(len, L'\0');
    state = std::mbstate_t();  // 重置转换状态
    src = utf8.c_str();
    std::mbsrtowcs(&wstr[0], &src, wstr.size(), &state);
    
    std::setlocale(LC_CTYPE, prev_locale.c_str());  // 恢复原始locale
    return wstr;
}
...
std::string wstring_to_utf8(const std::wstring& wstr) {
    ...
    size_t len = std::wcsrtombs(nullptr, &src, 0, nullptr);
    ...
    src = wstr.c_str();
    std::wcsrtombs(&utf8[0], &src, utf8.size(), nullptr);
...
//				std::string txt_uft8 = s2s(effective_ans.effective_word);
                std::string txt_uft8 = effective_ans.effective_word;
